...
program ivp2_FTCS
    ...
    ! Constants
    nx = 2001               ! Number of grid points
    nx_analytical = 4001   ! Higher resolution for analytical solution
    t_final = 01000.0       ! Final simulation time
    c = 1.0                ! Wave speed
    x_min = -1.0
    x_max = 1.0
    ...
    do iterations = 1, int(t_final / dt)
        ...
        do i = 2, nx-1
            f(i, 2) = f(i, 1) - 0.5 * CFL * (f(i + 1, 1) - f(i - 1, 1))
        end do
    ...
    open(11, file='FTCS_Solution.dat')
    do i = 1, nx
        write(11, *) x(i), f(i,2)
    end do
    close(11)
