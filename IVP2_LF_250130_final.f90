! IVP2 Lax-Friedrichs Implementation
program ivp2_lax_friedrichs
    implicit none
    ...
    do iterations = 1, int(t_final / dt)
        ...
        do i = 2, nx-1
            f(i, 2) = 0.5 * (f(i + 1, 1) + f(i - 1, 1)) - 0.5 * CFL * (f(i + 1, 1) - f(i - 1, 1))
        end do
    ...
    open(11, file='Lax_Friedrichs_Solution.dat')
    do i = 1, nx
        write(11, *) x(i), f(i,2)
    end do
    close(11)
...
end program ivp2_lax_friedrichs
